...
public class ProxyEvents {
    ...
    public void onProxyPing(ProxyPingEvent event) {
        ...
        if (VelocityCore.get().inMaintenanceMode()) {
            builder.description(VelocityCore.get().messages.getMessage(Messages.Keys.Message.MAINTENANCE_CURRENT_PING));
            builder.maximumPlayers(0);
            event.setPing(builder.build());
        } else event.getConnection().getVirtualHost().ifPresent(host -> {
            String domain = host.getHostString();

            String MOTD = VelocityCore.get().config().get("motd").getAsJsonObject().get(domain).getAsString();
            if (MOTD==null)
                return;
