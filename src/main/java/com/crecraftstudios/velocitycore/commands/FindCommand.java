                ...
                .then(BrigadierCommand.requiredArgumentBuilder("player", StringArgumentType.word())
                        ...
                        .executes(ctx -> {
                            ...
                            proxy.getPlayer(username).ifPresent(player -> {
                                ...
                                if (!server.isPresent())
                                    ctx.getSource().sendMessage(VelocityCore.get().messages.getMessage(Messages.Keys.Commands.FIND_ONLINE_BUT_NOT_IN_SERVER));//Component.text("Player is online but not in a server. Strange???"));
                                else {
                                    ctx.getSource().sendMessage(VelocityCore.get().messages.getMessage(Messages.Keys.Commands.FIND_ONLINE, username, server.get().getServerInfo().getName()));//Component.text(username+" is currently on "+server.get().getServerInfo().getName()));
                                }
                            });
