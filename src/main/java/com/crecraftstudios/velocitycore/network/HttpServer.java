package com.crecraftstudios.velocitycore.network;
...
public class HttpServer {
    ...
    private void runServer() {
        try {
            VelocityCore.get().logger.info("Starting HTTP server");
            this.server = com.sun.net.httpserver.HttpServer.create(new InetSocketAddress(VelocityCore.get().config().get("webserver").getAsJsonObject().get("host").getAsString(), VelocityCore.get().config().get("webserver").getAsJsonObject().get("port").getAsInt()), 0);
            ...
            try {
               if ("POST".equals(exchange.getRequestMethod())) {
                   ...
                   switch(path) {
                       ...
                       default -> VelocityCore.get().logger.info("{} what the heck is this path????", path);
                   }
        ...
        server.start();
        VelocityCore.get().logger.info("HTTP server started");
    }
    ...
    private void handleRegister(JsonObject json) {
        ...
        VelocityCore.get().proxy.getServer(name).orElseGet(()-> {
            VelocityCore.get().logger.info("Registering server {} with host {}", name, host);
            return VelocityCore.get().proxy.registerServer(new ServerInfo(name, new InetSocketAddress(host, port)));
        });
    ...
    private void handleUnregister(String name) {
        VelocityCore.get().proxy.getServer(name).ifPresent((server)-> VelocityCore.get().proxy.unregisterServer(server.getServerInfo()));
    }

    private void handleOffline(String name) {
        VelocityCore.get().serverIsOffline(name);
    }

    private void handleOnline(String name) {
        VelocityCore.get().serverIsOnline(name);
    }
}